...
export const useHomeStore = defineStore("homes", () => {
  ...
  async function getHome(creator, homeName) {
    const res = await axios.get(
      `${import.meta.env.VITE_API_BASE}/${creator}/${homeName}`,
  ...
  async function joinHome(creator, homeName, inviteId) {
    return (
      await axios.put(
        ...
        }/${creator}/${homeName}/join?invite_id=${inviteId}`,
        { headers: { Authorization: `Bearer ${user.accessToken}` } }
      )
    ).data;
  }
  ...
  async function createInviteLink(creator, homeName) {
    return (
      await axios.post(
        `${import.meta.env.VITE_API_BASE}/${creator}/${homeName}/invite`,
        { headers: { Authorization: `Bearer ${user.accessToken}` } }
      )
    ).data;
  }

  return { getHome, createHome, createInviteLink, joinHome };
});
